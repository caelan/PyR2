...
from ss_light.problems import dantam2
from ss_light.utils import is_window_active, inverse_kinematics, Traj, \
...
def amber_stripstream(use_window=True, step=False):
  ...
  def cfree_traj(traj, pose): # Collision free test between a robot executing traj (which may or may not involve a grasp) and an object at pose
    ...
    if traj.grasp is not None:
      attached = {ARM: body.applyTrans(get_wrist_frame(traj.grasp, robot, ARM).inverse())}
    collision_fn = get_collision_fn(placed, attached) # TODO - cache bodies for these configurations
  ...
  def sample_grasp_traj(pose, grasp): # Sample pregrasp config and motion plan that performs a grasp
    attached = {ARM: body.applyTrans(get_wrist_frame(grasp, robot, ARM).inverse())}
    collision_fn = get_collision_fn(static_bodies, attached)

    manip_frame = manip_from_pose_grasp(pose, grasp, robot, ARM)
    grasp_conf = next(inverse_kinematics(problem.initial_conf, manip_frame, ARM), None) # Grasp configuration
    ...
    pregrasp_conf = next(inverse_kinematics(grasp_conf, pregrasp_frame, ARM)) # Pregrasp configuration
  ...
  def sample_free_motion(conf1, conf2): # Sample motion while not holding
    ...
    path = birrt(conf1, conf2, get_distance_fn(active_chains),
                 ...
                 get_extend_fn(active_chains, STEP_SIZE), get_collision_fn(static_bodies),
  ...
  def sample_holding_motion(conf1, conf2, grasp): # Sample motion while holding
    ...
    path = birrt(conf1, conf2, get_distance_fn(active_chains),
                 get_sample_fn(conf1, active_chains),
                 get_extend_fn(active_chains, STEP_SIZE), get_collision_fn(static_bodies, attached),
                 restarts=0, iterations=50, smooth=None)
    if path is None:
      return
  ...
  constants = map(GRASP, TOP_GRASPS[:MAX_GRASPS]) + map(POSE, problem.known_poses)
  initial_atoms = [
  ...
  goal_literals = [PoseEq(obj, pose) for obj, pose in problem.goal_poses.iteritems()]
  if problem.goal_conf is not None:
    goal_literals.append(ConfEq(problem.goal_conf))
  stream_problem = STRIPStreamProblem(initial_atoms, And(*goal_literals), actions + axioms, cond_streams, constants)
  ...
  if is_window_active(window_3d) and plan is not None:
    ...
    placements = {get_name(body): body.applyTrans(pose) for body, pose in problem.initial_poses}
    ...
    for i, (action, args) in enumerate(plan):
      if action.name == 'move':
        ...
        attached[ARM] = name_to_body[obj].applyTrans(get_wrist_frame(grasp, robot, ARM).inverse())
        del placements[obj]
        _execute_traj(traj.path)
