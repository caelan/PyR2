...
def approach_from_manip(manip_trans):
  if abs(manip_trans.matrix[2,0]) < 0.1: # Ad-hoc backoff strategy
    return manip_trans.compose(hu.Pose(-BACKOFF,0.,PERP_BACKOFF,0.)) # horizontal
...
def get_extend_fn(active_chains, step_size):
  #return lambda q1, q2: interpolatePath([q1, q2])
...
def get_distance_fn(active_chains):
  def distance_fn(q1, q2):
    ...
    #return q1.robot.distConfAbs(q1, q2, active_chains, float('inf'))
    #return q1.distance(q2, active_chains)
  return distance_fn
...
def get_collision_fn(placed=[], attached=None):
  ...
  def collision_fn(q):
    ...
    #if any(placed_robot.collides(body) for body in placed):
    #  for body in placed:
    #    if placed_robot.collides(body):
    #      body.draw('Window')
    #  placed_robot.draw('Window')
    #  raw_input('Continue?')
